...
class PaymentAttempt(Document):
    ...
    user: Link[User]
    amount: float
    currency: str # e.g., "usd", "usdt"
    payment_method: Literal["card", "usdt"]
    payment_processor: Optional[str] = None # e.g., "stripe"
    transaction_id: Optional[str] = None # From payment processor or blockchain
    status: Literal["pending", "succeeded", "failed", "requires_action"] = "pending"
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    metadata: Optional[dict] = None # e.g., Stripe payment intent ID, error messages
...
class Subscription(Document):
    ...
    payment_processor_subscription_id: Optional[str] = None # e.g., Stripe subscription ID
